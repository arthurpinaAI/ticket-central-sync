#!/usr/bin/env python3
"""
Ticket Central — Zapier/Make-style one-shot sync (composite-key dedupe, 429-proof markers)
...
- Unique row key = SHA1(source_id + flow + required-columns):
    ALL key = B + C
    LI  key = B + C + D
- Appends mapped rows to Master: Tickets, once per unique key (stored in __KeyIndex).
- "Start now" baseline: set START_FROM_NOW=true for first run → records current keys as seen
  (no appends) and marks baseline in __Markers. Subsequent runs append only brand-new rows.
- Optimized to avoid Google 429:
  * Batching & backoff on reads/writes
  * __Markers is read ONCE per run, and writes are append-only (no read on write)
"""
...
BATCH_APPEND_ROWS     = int(os.getenv("BATCH_APPEND_ROWS", "500"))# rows per append
...
# Composite key columns (use required columns)
KEYCOLS_ALL = [2, 3]        # B + C
...
# --- Markers: read once, write append-only (no read on write) ---

def markers_get_map_safe(markers_ws) -> Dict[str, str]:
    """
    Load __Markers into a dict once: {key: last_value}. Backoff on 429.
    """
    delay = BACKOFF_BASE_SEC
    for _ in range(6):
        try:
            vals = markers_ws.get_all_values()
            out = {}
            for r in vals[1:]:
                if not r: continue
                k = r[0]
    ...
    for r in vals[1:]:
        if not r: continue
        k = r[0]; v = r[1] if len(r)>1 else ""
        if k: out[k] = v
    return out

def write_marker_append_only(markers_ws, markers_cache: Dict[str, str], key: str, value: str):
    """
    Append-only write to __Markers (no read). Update cache in-memory.
    """
    ts = now_utc()
    markers_ws.append_row([key, str(value), ts], value_input_option="RAW")
    markers_cache[key] = str(value)
...
def process_flow_for_source(
    ...
    # Per-flow config
    if flow == "ALL":
        tab, start_row, required, mapping, statics, keycols = TAB_ALL, START_ROW_ALL, REQ_ALL, MAP_ALL, {}, KEYCOLS_ALL
    else:
        tab, start_row, required, mapping, statics, keycols = TAB_LI, START_ROW_LI, REQ_LI, MAP_LI, STATIC_LI, KEYCOLS_LI

    ss = gc.open_by_key(spreadsheet_id)
    ws = get_ws(ss, tab)
    if not ws:
        write_marker_append_only(markers_ws, markers_cache, f"BASELINE::{flow}::{spreadsheet_id}", f"NO_TAB:{tab}")
        return 0, 0
    ...
    already_baselined = baseline_key in markers_cache
    ...
    while r <= max_row:
        ...
        for i, row in enumerate(values):
            ...
            if START_FROM_NOW and not already_baselined:
                # Baseline: record key only (no Tickets append)
                if k not in seen:
    ...
    # Flush remaining & mark
    if START_FROM_NOW and not already_baselined:
        if batch_keys:
            append_rows_safe(key_ws, batch_keys, "RAW")
            batch_keys.clear()
        write_marker_append_only(markers_ws, markers_cache, baseline_key, f"DONE:{max_row}")
    else:
        if batch_rows:
            append_rows_safe(tickets_ws, batch_rows, "RAW")
            append_rows_safe(key_ws,     batch_keys, "RAW")
            total_appended += len(batch_rows)
        write_marker_append_only(markers_ws, markers_cache, f"CURSOR::{flow}::{spreadsheet_id}", f"DONE:{max_row}")
...
def main():
    ...
    # Load markers ONCE with backoff; then use append-only writes
    markers_cache = markers_get_map_safe(markers_ws)
    ...
    for flow in (FLOW_ALL, FLOW_LI):
        ...
        for sid in sources:
            a, s = process_flow_for_source(gc, tickets_ws, key_ws, markers_ws, markers_cache, master_width, sid, flow)
